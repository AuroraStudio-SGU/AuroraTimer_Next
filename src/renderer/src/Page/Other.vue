  ...
  <div class="menu">
    <div class="white-box">
      ...
      <div class="between">
        ...
        <div>
          <span class="label-text">本学期的打卡情况</span>
          <el-switch
              v-model="isShowTable"
              :active-icon="Check"
              :inactive-icon="Close"
...
import {ref} from "vue";
import Plotly from "plotly.js-dist"; //纯js不支持ts定义
...
const loadAnimation = async () => {
  ...
  let json;
  let res = await getTimeDetail(globalStore.Setting.userInfo.id);
  if(!res.success){
    ElNotification({
      title: "请求失败！",
      message: res.msg,
  ...
  for (let i = 0; i < json.dates.length; i++) {
    let d = json.dates[i];
    let date = new Date(d)
    json.dates[i] = `${date.getFullYear()}-${((date.getMonth() + 1))}-${(date.getDate())}`
  }
  const frames = [];
  const x = json.dates;
  const y = json.times;
  ...
  const layout = {
    title: '打卡程度表',
    xaxis: {
      range: [frames[frames.length-1].data[0].x[0], frames[frames.length-1].data[0].x[x.length-1]],
      showgrid: false
    },
    yaxis: {
      range: [0, Math.max(...y)],
